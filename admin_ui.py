"""
Streamlit admin UI: upload files, choose schema, set mandatory parameters, run validation.
Two tabs: Tabular Data | Spatial Data.
Persists data through Bronze -> Silver -> Gold layers.
"""
import tempfile
...
with tab_tabular:
    ...
    if upload_tabular and mandatory_fields:
        ...
        if st.button("Validate & Save", key="btn_validate_tabular"):
            # --- Create run folders ---
            layers = create_layered_run(schema_choice)
            bronze_artifacts = []
            silver_artifacts = []
            gold_artifacts = []
            metrics = None

            # --- Bronze: save raw upload ---
            ...
            if read_err:
                ...
                # --- Silver: save validated data + report ---
                report_path = save_silver_validation_report(
                    layers["silver"],
                    schema_choice,
                    total_rows=total_rows,
    ...
    if upload_spatial is not None:
        ...
        if preview_err:
            ...
            if st.button("Validate & Save", key="btn_validate_spatial"):
                layers = create_layered_run("spatial_data")
                bronze_artifacts = []
                silver_artifacts = []
                gold_artifacts = []
                ...
                if result["error"]:
                    ...
                    # ---- Silver: save validated GeoParquet + report ----
